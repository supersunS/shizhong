...
- (void)viewDidLoad
{
    ...
    _sz_nav=[[likes_NavigationView alloc]initWithTitle:@"设置" andLeftImage:[UIImage imageNamed:@"return"] andRightImage:@"保存" andLeftAction:^{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        if (indexPath.row == 0) {
            ...
            self.pushDisplaySwitch.frame = CGRectMake(_tableView.frame.size.width - self.pushDisplaySwitch.frame.size.width - 10, (cell.contentView.frame.size.height - self.pushDisplaySwitch.frame.size.height) / 2, self.pushDisplaySwitch.frame.size.width, self.pushDisplaySwitch.frame.size.height);
