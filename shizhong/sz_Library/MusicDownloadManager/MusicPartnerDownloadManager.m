...
+ (instancetype)sharedInstance
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        mpartnerDownloadManager.maxDownloadCount = 1;
    });
...
- (void)start:(NSString *)url withExt:(NSDictionary *)ext
{
    MusicPartnerDownloadTask *downLoadTask = [self getDownLoadTask:url];
    if(downLoadTask)
    {
...
//        [self deleteFile:url];
        ...
        downLoadEntity.downLoadUrlString = url;
        downLoadEntity.extra = ext;
...
//            [self start:task.mpSessionModel.urlString];
