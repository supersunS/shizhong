...
@property (strong, nonatomic) NSMutableDictionary *defaultEmojiTitle;
...
{
    self = [self initWithFrame:frame horizontalPadding:8 verticalPadding:5 inputViewMinHeight:36 inputViewMaxHeight:150 type:EMChatToolbarTypeGroup nomal:NO];
    if (self) {
...
{
    self = [self initWithFrame:frame horizontalPadding:8 verticalPadding:5 inputViewMinHeight:36 inputViewMaxHeight:150 type:type nomal:NO];
    if (self) {
                        ...
                        nomal:(BOOL)nomal
{
    ...
    if (self) {
        ...
        _defaultEmojiTitle=[EaseEmoji allEmojiTitle];
...
- (void)_setupSubviews
{
    ...
    _backgroundImageView.image = [UIImage createImageWithColor:sz_bgColor];
    ...
    _inputTextView.placeHolder =@"请输入评论内容";
    ...
    if(_normal)
    {
        ...
        [self.sendButton setImage:[UIImage createImageWithColor:sz_bgDeepColor] forState:UIControlStateNormal];
        self.sendButton.titleLabel.font=sz_FontName(12);
        ...
        EaseChatToolbarItem *sendItem = [[EaseChatToolbarItem alloc] initWithButton:self.sendButton withView:nil];

        [self setInputViewLeftItems:@[faceItem]];
        [self setInputViewRightItems:@[sendItem]];
    }
    else
    {
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
    ...
    {
        [self selectedFacialView:nil isDelete:YES];
        return NO;
    }
    return YES;
}
...
//        self.inputTextView.text = [NSString stringWithFormat:@"%@%@",chatText,str];
        ...
        if (chatText.length > 0) {
            ...
            NSRange range = [[resultArray lastObject] range];
            //获取原字符串中对应的值
            NSString *subStr = [chatText substringWithRange:range];
            if (chatText.length >= subStr.length  && [[chatText substringWithRange:NSMakeRange(chatText.length-subStr.length, subStr.length)]isEqualToString:subStr]) {
                if ([_defaultEmojiTitle objectForKey:subStr]) {
...
- (void)faceButtonAction:(id)sender
{
    ...
    NSMutableArray *itemArray=self.rightItems;
    if(_normal)
    {
        itemArray=self.leftItems;
    }
    for (EaseChatToolbarItem *item in itemArray)
    {
        if (item.button == button){
            faceItem = item;
            continue;
