...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _facesTitleDict=[[NSMutableDictionary alloc]initWithDictionary:[EaseEmoji allEmojiTitle]];
            ...
            for (int col = 0; col < maxCol; col++) {
                ...
                if (index < [_faces count]) {
                    ...
                    if (emotionManager.emotionType == EMEmotionGif) {
                        ...
                        [button setImage:[UIImage imageNamed:[NSString stringWithFormat:@"%@",[_faces objectAtIndex:index]]] forState:UIControlStateNormal];
...
-(void)selected:(UIButton*)bt
{
    if (bt.tag == 10000 && _delegate) {
        ...
        NSString *key=[NSString stringWithFormat:@"%@",[_faces objectAtIndex:bt.tag]];
        NSString *str = [_facesTitleDict objectForKey:key];
        if (_delegate) {
