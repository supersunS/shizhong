    ...
    if (self) {
        ...
        switch (_firstMessageBody.messageBodyType) {
            case eMessageBodyType_Text:
            {
                ...
                // 表情映射。
//                NSString *didReceiveText = [EaseConvertToCommonEmoticonsHelper convertToSystemEmoticons2:textBody.text];
//                self.attrBody = [EaseEmotionEscape attStringFromTextForChatting:didReceiveText];
//                self.text = didReceiveText;
                self.attrBody=[EaseConvertToCommonEmoticonsHelper convertToSystemEmoticonsTitle:textBody.text];
            }
                break;
