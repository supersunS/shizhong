...
- (void)commonInit
{
    if ([UIDevice currentDevice].systemVersion.floatValue >= 7.0) {
        [[UINavigationBar appearance] setBarTintColor:sz_bgColor];
...
-(void)willAutoLoginWithInfo:(NSDictionary *)loginInfo error:(EMError *)error
{
    ...
    {
        //   自动回调失败重新登录环信
        [sz_loginAccount loginEase];
    }
    else
    {
        EMError *error = [[EaseMob sharedInstance].chatManager loadDataFromDatabase];
    }
}
...
{
    // 表情映射。
//    NSString *willSendText = [EaseConvertToCommonEmoticonsHelper convertToCommonEmoticons2:text];
...
{
    NSLog(@"当前账号已在其他设备登录");
    UIAlertView *alert=[[UIAlertView alloc]initWithTitle:@"提示" message:@"你的账号已在其他设备登陆" delegate:self cancelButtonTitle:@"确定" otherButtonTitles: nil];
    [alert show];
...
{
    [[NSNotificationCenter defaultCenter]postNotificationName:SZ_NotificationLoginStatusChange object:@NO];
}
